...
export async function reserveMaterialHash(contentHash: string, owner: Owner, now = Date.now()) {
  ...
  if (!existing) throw new Error("材料哈希预留状态不一致");
...
export async function commitMaterialHashReservation(contentHash: string, materialId: string) {
  await db.update(materialHashReservations).set({ state: "committed", leaseUntil: null }).where(and(
...
export async function reconcileMaterialHashReservations(now = Date.now()) {
  await db.transaction(async (tx) => {
    ...
    const stalePending = await tx.select().from(materialHashReservations).where(and(
      eq(materialHashReservations.state, "pending"),
      lte(materialHashReservations.leaseUntil, now),
    ));
