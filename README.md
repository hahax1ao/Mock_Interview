...
- 已完成智能解析的旧个人材料可使用“重新提取详细经历”，补建科研、项目与竞赛经历卡片；
- 详细经历草稿可逐项编辑；只有确认整张卡片后，才会成为面试追问和复盘的证据；
- 项目科研面试会优先围绕已确认且与考察重点最相关的一张经历卡追问；
- 10/20/30 分钟四角色实时面试；
...
材料先在本机提取文本和建立索引。需要智能解析时，只把提取后的文本发送到阿里云百炼；原始 PDF、DOCX 或图片文件不会发送到百炼。原始文件、详细经历卡片、面试转写和复盘数据均保存在本机，请勿把真实 API Key 提交到版本库。
